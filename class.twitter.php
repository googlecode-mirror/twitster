...
class twitter{
	...
	function search($qs) {
	  $request = "http://search.twitter.com/search.atom?$qs";
	  return $this->objectify( $this->process($request) );
	}	
	...
	{
		$request = 'http://twitter.com/account/verify_credentials.' . $this->type;
		$response = $this->process( $request, false, true );
	 ...
	 */
	function process($url,$postargs=false,$force_return=false)
	{
	  $ch = curl_init($url);
	  if($postargs !== false)
	    {
	  ...
	  if( intval( $this->responseInfo['http_code'] ) == 200 || $force_return )
	    return $response;    
	  else
            return false;
	}
	...
	function objectify( $data )
	{
	  if( $this->type ==  'json' )
	      ...
	      if( function_exists('simplexml_load_string') )
		{
		  ...
		  $obj_status or $obj_status = array();
		  
		  foreach( $obj->entry as $status )
		    {
		      $statuses[] = $status;
		    }
	      ...
	      if( function_exists('simplexml_load_string') )
		{
		  ...
		  if ($obj->status) {
		    foreach( $obj->status as $status ) { $statuses[] = $status; }
		  } else if ($obj->user) {
		    foreach( $obj->user as $user ) { $statuses[] = $user; }
		  }
		  return (object) $statuses;
